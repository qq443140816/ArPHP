...
class ArDbMysql extends ArDb
{
    ...
    private function query($sql = '')
    {
        ...
        if(in_array($sqlCmd, array('UPDATE', 'DELETE')) && stripos($sql, 'where') === false)
            throw new \Core\ArDbException('no WHERE condition in SQL(UPDATE, DELETE) to be executed! please make sure it\'s safe', 42005);
    ...
    public function delete()
    {
        ...
        if (!preg_match('/ WHERE /i', $sql))
            throw new \Core\ArDbException('bad sql condition , where must to be infix');
        return $this->exec($sql);
    ...
    public function buildCondition($condition = array(), $logic = 'AND')
    {
        if( ! is_array($condition))
        {
            if (is_string($condition))
            {
                ...
                if(!$count)
                {
                    throw new \Core\ArDbException('bad sql condition: must be a valid sql condition');
                }
            ...
            throw new \Core\ArDbException('bad sql condition: ' . gettype($condition));
        }
                ...
                if($v_connect == ' BETWEEN') {
                    ...
                    if (empty($v_connect)) {
                        if($this->allowGuessConditionOperator === null || $this->allowGuessConditionOperator === true)
                        {
                            ...
                            throw new \Core\ArDbException("guessing condition operator is not allowed: use '$k IN'=>array(...)");
                    }
                }
                else if (empty($v)) {
                    // 'key' => array()
