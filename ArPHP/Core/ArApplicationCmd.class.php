...
class ArWorker
{
    ...
    public static function deamon()
    {
        ...
        $pid = pcntl_fork();
        if ($pid == -1) :
            throw new ArExceptionCmd("fork faild", 1200);
        endif;
        if ($pid > 0) :
            exit(0);
        else :
            if (posix_setsid() == -1) :
                throw new ArExceptionCmd("set leader sid faild", 1201);
            endif;
            // 脱离终端控制
            $pid = pcntl_fork();
            if ($pid == -1) :
            ...
            if ($handle) :
                ...
                throw new ArExceptionCmd('can not open stdoutFile ' . $stdoutFile);
            endif;
        endif;
    ...
    public function stop()
    {
        ...
        if ($this->onStop) :
            try {
                ...
                echo $e->getMessage();
                arComp('list.log')->record($e->getMessage(), 'errorstop');
            }
        endif;
                ...
                foreach (self::$pids as $workerId => $workerPidArray) :
                    if (isset($workerPidArray[$pid])) :
                        ...
                        if (self::$_status !== self::STATUS_SHUTDOWN) :
                            arComp('list.log')->record("worker[".$worker->socketName.":$pid] exit with status $status", 'worker_exit');
                            self::forkWorkers();
                        endif;
                        break;
                    endif;
    ...
    public function acceptConnection($socketServer)
    {
        ...
        if ($this->onConnect) :
            try {
                ...
                echo $e->getMessage();
                arComp('list.log')->record($e->getMessage(), 'exceptionconnect');
...
class ArTcpConnection
{
                ...
                if ($protocol = $this->worker->protocol) :
                    ...
                    if ($length === 0) :
                        ...
                        if ($length > 0 && $length <= self::$maxSendBufferSize) :
                            ...
                            arComp('list.log')->record(time(), 'error_package_length');
                            $this->close();
                            return;
                        endif;
                        ...
                        if ($message === false) :
                            ...
                            if ($this->worker->onMessage) :
                               ...
                               try {
                                   ...
                                   echo $e->getMessage();
                                   // 日志
                                   arComp('list.log')->record($e->getMessage(), 'readprotocol');
                    ...
                    if ($this->worker->onMessage) :
                       ...
                       try {
                           ...
                           echo $e->getMessage();
                           arComp('list.log')->record($e->getMessage(), 'readnoprotocol');
                           self::$statistics['throw_exception']++;
                       }
                    endif;
                    $this->readBuffer = '';
    ...
    {
        // arComp('list.log')->record(strlen($this->writeBuffer), 'buffer' . (int)$this->clientSocket);
        if ($this->writeBuffer !== '') :
    ...
    public function send($message, $raw = false)
    {
        ...
        if ($this->writeBuffer === '') :
            ...
            if (self::$maxSendBufferSize <= strlen($this->writeBuffer) + strlen($message)) :
                // 为status命令统计发送失败次数
                self::$statistics['send_fail']++;
                arComp('list.log')->record('cs:' . (strlen($this->writeBuffer) + strlen($message)) . '>' . self::$maxSendBufferSize . ' msg : ' . $message, 'msg_full' . __FUNCTION__);
                //arComp('list.log')->record($this->writeBuffer, 'msg_fulling_writebuffer' . __FUNCTION__);
                // 如果有设置失败回调，则执行
    ...
    public function destruct()
    {
        if ($this->worker->onClose) :
            try {
                ...
                arComp('list.log')->record($e->getMessage(), 'exceptionclose');
                self::$statistics['throw_exception']++;
            }
        endif;
